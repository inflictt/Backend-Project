// importing cloudinary v2 SDK used for uploading files/images/videos to cloudinary
import { v2 as cloudinary } from "cloudinary";
// fs = file system module used for deleting temporary local files
import fs from "fs";
...
const uploadOnCloudinary = async (localFilePath) => {
    try {
        // if file path not received return null immediately
        ...
        const response =
            await cloudinary.uploader.upload(
                localFilePath,
                {
                    // automatically detects:image / video / raw file
                    resource_type: "auto",
                }
            );
